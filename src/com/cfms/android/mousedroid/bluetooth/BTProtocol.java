...
public class BTProtocol {
	...
	public static void onMouseButtonEvent(BluetoothService btService,
			MouseButton MB, MouseButtonEvent MBE) {
		byte[] packet = { PACKET_PREAMBLE,
				PacketID.MOUSE_BUTTON_EVENT.getCode(), MB.getCode(),
				MBE.getCode(), CR, LF };
		btService.write(packet, packet.length);
	}

	public static void onMoveMouse(BluetoothService btService, int dx, int dy) {
		byte[] packet = { PACKET_PREAMBLE, PacketID.MOVE_MOUSE.getCode(), 0, 0,
				0, 0, CR, LF };
		packet[2] = (byte) (dx & 0xff);
		packet[3] = (byte) ((dx >> 8) & 0xff);
		packet[4] = (byte) (dy & 0xff);
		packet[5] = (byte) ((dy >> 8) & 0xff);
		btService.write(packet, packet.length);
	}
